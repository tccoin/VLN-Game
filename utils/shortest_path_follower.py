...
class ShortestPathFollowerCompat:
    ...
    def get_next_action(
        ...
        if max_grad_dir is None:
            return self._get_return_value(HabitatSimActions.TURN_LEFT)
    ...
    def _geo_dist(self, goal_pos) -> float:
    ...
    def get_path_points(self, goal_pos):
        return self._sim.get_straight_shortest_path_points(
            self._sim.get_agent_state().position, self.get_closet_navigable_point(goal_pos)
        )
    
    def _est_max_grad_dir(self, goal_pos) -> np.array:
        ...
        if self.mode == "geodesic_path":
            ...
            if len(points) < 2:
                return None
            points_error = points[1] - points[0]
            points_error[1] = 0
