...
from utils.general_utils import to_tensor, to_numpy
...
def get_camera_K(width, height, fov):
    ...
    camera_matrix = {'cx': cx, 'cy': cy, 'fx': fx}
...
def filter_objects(cfg, objects: MapObjectList):
    ...
    objects_to_keep = []
    for obj in objects:
        if len(obj['pcd'].points) >= cfg.obj_min_points and (obj['num_detections'] >= cfg.obj_min_detections or 'stairs' in obj['class_name']):
